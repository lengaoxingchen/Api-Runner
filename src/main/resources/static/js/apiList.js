...
function pagingDataBuild(pagedata){
	...
	for(var i=0;i<pagedata.length;i++){
		...
		html += '<li><a href="#" onclick="showApi('+pagedata[i].id+')">'+pagedata[i].name+'</a></li>';
...
$(function(){
	...
	//获取数据接口
	if(projectId!=""){
		listUrl = lemon.config.global.rootUrl+"/api/showApiUnderProject?projectId="+projectId;
	}else if(apiClassificationId!=''){
		listUrl = lemon.config.global.rootUrl+"/api/showApiUnderApiClassification?apiClassificationId="+apiClassificationId;
	}
	...
	$('.btn-addinter').click(function(){
		var url = lemon.config.global.rootUrl+"/apiClassification/findAll";
		//准备分类下拉框数据
