...
$(function(){
	//共选择的集合
	var url = lemon.config.global.rootUrl+"/suite/listAll";
		...
		var dialog = jqueryAlert({
		    ...
		    'buttons' :{
		        ...
		        '确定':function(){
		        	...
		        	var url=lemon.config.global.rootUrl+"/case/add?name="+name+"&suiteId="+suiteId;
		        	$.ajax({
		        		url:url,
	...
	$(".btn-send").click(function(){
		$.ajax({
			url:lemon.config.global.rootUrl+"/api/run",
			data:$("[name='apiRunForm']").serialize(),
			type:'post',
			dataType:'json',
			success:function(ret){
				if(ret.status=="1"){
					$("[name='responseHeader']").html("<pre>"+ret.data.responseHeaders+"</pre>");
